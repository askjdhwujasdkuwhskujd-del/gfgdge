...
const popularTags = ["games", "podcasts", "live", "creators", "documentary", "design", "motion", "latam", "platform", "ui", "recent-uploads"];
...
async function renderFeed(options = {}) {
  ...
  const isHome = !options.search && !options.feed && !options.tag;
  const mainVideos = data.videos.slice(0, isHome ? 6 : data.videos.length);
  const shortVideos = isHome ? data.videos.slice(0, 6) : [];
  ...
  app.innerHTML = `
    ...
    <nav class="tag-strip" aria-label="Popular tags">
      <button class="chip ${!options.tag ? "active" : ""}" data-home="true" type="button">All</button>
      ${popularTags.map((tag) => `<button class="chip ${options.tag === tag ? "active" : ""}" data-tag="${tag}" type="button">#${tag}</button>`).join("")}
    </nav>
    ${mainVideos.length ? `<section class="video-grid">${mainVideos.map(videoCard).join("")}</section>` : emptyState("No frames yet", "Upload the first public video for this view.")}
    ${shortVideos.length ? `
      <section class="shorts-section">
        <div class="shorts-head">
          <span class="shorts-mark">▱</span>
          <h2>Shorts</h2>
        </div>
        <div class="shorts-grid">
          ${shortVideos.map(shortCard).join("")}
        </div>
      </section>
    ` : ""}
  `;
  ...
  const allButton = app.querySelector("[data-home]");
  if (allButton) {
...
function shortCard(video) {
  return `
    <article class="short-card">
      <a class="short-thumb" href="#/watch/${video.id}" aria-label="Watch ${escapeAttr(video.title)}">
        <img src="${escapeAttr(video.poster || "/assets/poster-fallback.svg")}" alt="">
        <span class="duration">${escapeHtml(video.duration || "new")}</span>
      </a>
      <h3><a href="#/watch/${video.id}">${escapeHtml(video.title)}</a></h3>
