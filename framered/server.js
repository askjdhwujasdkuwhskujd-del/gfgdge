...
async function handleApi(req, res, url) {
  ...
  if (req.method === "POST" && pathname === "/api/auth/register") {
    ...
    const handle = cleanHandle(body.handle || name);
    const email = cleanEmail(body.email);
    const password = String(body.password || "");

    if (!name || !handle || !email || password.length < 6) {
      sendJson(res, 400, { error: "Use a name, handle, valid email, and password with at least 6 characters." });
      return;
    }

    if (db.users.some((candidate) => candidate.email === email || candidate.handle === handle)) {
      sendJson(res, 409, { error: "That email or handle is already taken." });
      return;
    }
  ...
  if (req.method === "POST" && pathname === "/api/auth/login") {
    ...
    const email = cleanEmail(body.email);
    const password = String(body.password || "");
    const matchedUser = db.users.find((candidate) => candidate.email === email);
...
function seedDb() {
  const demoUser = {
    id: "usr_demo",
    name: "Framered Studio",
    handle: "framered",
    email: "demo@framered.lat",
    passwordHash: hashPassword("framered"),
    avatar: "FR",
    bio: "Launch notes, creator tools, and sharp frames from the Framered team.",
    createdAt: new Date().toISOString(),
    subscribers: ["usr_maya", "usr_renato"],
    subscriptions: ["usr_maya"]
  };
  ...
  const maya = {
    ...
    passwordHash: hashPassword(makeId("pw")),
    avatar: "MC",
    bio: "Documentary cuts and city notebooks.",
    createdAt: new Date().toISOString(),
    subscribers: ["usr_demo"],
    subscriptions: ["usr_demo"]
  };
  ...
  const renato = {
    ...
    passwordHash: hashPassword(makeId("pw")),
    avatar: "RA",
    bio: "Design, motion tests, and product ideas.",
    createdAt: new Date().toISOString(),
    subscribers: [],
    subscriptions: ["usr_demo"]
  };
  ...
  return {
    users: [demoUser, maya, renato],
    videos: [
      seededVideo("vid_launch", demoUser.id, "Welcome to Framered", "A fast tour of the creator-first video space being built for framered.lat.", ["launch", "platform", "creators"], "/assets/poster-launch.svg", 438),
      seededVideo("vid_city", maya.id, "Sao Paulo in 90 seconds", "Street textures, transit rhythms, and a compact visual essay.", ["documentary", "cities", "latam"], "/assets/poster-city.svg", 1280),
      seededVideo("vid_motion", renato.id, "Interface motion studies", "Small transitions that make uploads, comments, and watch pages feel alive.", ["design", "motion", "ui"], "/assets/poster-motion.svg", 876)
    ],
    sessions: {}
  };
}

function seededVideo(id, ownerId, title, description, tags, poster, views) {
  return {
    id,
    ownerId,
    title,
    ...
    comments: [
      {
        id: makeId("cmt"),
        userId: "usr_demo",
        text: "This is a seeded demo frame. Upload a real video to replace it with playback.",
        createdAt: new Date().toISOString()
      }
    ],
    createdAt: new Date(Date.now() - Math.floor(Math.random() * 86400000 * 8)).toISOString()
  };
}
